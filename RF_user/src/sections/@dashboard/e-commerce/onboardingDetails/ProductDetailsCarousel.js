...
export default function ProductDetailsCarousel({ product }) {
  ...
  const imagesLightbox = product.filesImage?.map((img) => ({ src: img.preview })) || [];
  ...
  const carouselSettings2 = {
    ...
    variableWidth: true,
    centerPadding: '0px',
    slidesToShow: product.filesImage?.length > 3 ? 3 : product.filesImage?.length || 0,
  };
  ...
  const renderLargeImg = (
    <Box sx={{ mb: 3, borderRadius: 2, overflow: 'hidden', position: 'relative' }}>
      <Carousel {...carouselSettings1} asNavFor={nav2} ref={carousel1}>
        {product.filesImage?.map((img) => (
          <Image
            key={img}
            alt="product"
            src={img.preview}
            ratio="1/1"
            onClick={() => handleOpenLightbox(img.preview)}
            sx={{ cursor: 'zoom-in' }}
          />
        ))}
      </Carousel>
      ...
      <CarouselArrowIndex
        index={currentIndex}
        total={product.filesImage?.length || 0}
        onNext={handleNext}
  ...
  const renderThumbnails = (
    <StyledThumbnailsContainer length={product.filesImage.length}>
      <Carousel {...carouselSettings2} asNavFor={nav1} ref={carousel2}>
        {product.filesImage?.map((img, index) => (
          <Image
            key={img}
            disabledEffect
            alt="thumbnail"
