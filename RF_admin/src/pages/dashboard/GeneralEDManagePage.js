...
export default function GeneralEDManagePage() {
  ...
  const { EDs } = useSelector((state) => state.ed);
    ...
    <>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <Stack flexDirection={isDesktop ? 'row' : 'column'} justifyContent="space-between" mb={1}>
          <Typography variant="h4">RealityFence ED</Typography>
          <Stack flexDirection="row" gap={1}>
            <Button
              variant="outlined"
              onClick={() => {
                navigate(PATH_DASHBOARD.general.ED.draft);
              }}
            >
              Drafts
            </Button>
            <Button
              variant="outlined"
              onClick={() => {
                navigate(PATH_DASHBOARD.general.ED.newED);
              }}
            >
              New Post
            </Button>
